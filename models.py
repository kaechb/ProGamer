...
class Block(nn.Module):
    def __init__(self, embed_dim=60, num_heads=4,hidden=60,
        ...
        self.attn = spectral_norm(nn.MultiheadAttention(embed_dim,num_heads,batch_first=True,bias=bias),"in_proj_weight")
...
class Gen(nn.Module):
    def __init__(self,n_dim,l_dim_gen,hidden_gen,num_layers_gen,heads_gen,proj_gen,dropout_gen,activation_gen,latent,n_part,affine,bias,slope,res,**kwargs):
        super().__init__()
        self.slope=slope
...
class Disc(nn.Module):
    def __init__(self,n_dim,l_dim,hidden,num_layers,heads,dropout,proj,activation,slope,affine,bias,**kwargs):
        ...
        self.hidden = spectral_norm(nn.Linear(l_dim , int(hidden),bias=bias ))

        self.hidden2 = spectral_norm(nn.Linear(int(hidden), l_dim,bias=bias ))
